...
public class Club {
    ...
    public void startTournament() {
        ...
        for (Archer archer : archers) {
            if (!contestants.containsKey(archer.getType())) {        //Check if there is a key...
                contestants.put(archer.getType(), new HashSet<>());  //...if not -> create one + value
            } else {
                contestants.get(archer.getType()).add(archer);            //Add an archer in the correct key type
            }
            archer.attend();
            archer.shootArrows();
        }
    ...
    private void printMenWithCarbonByExp() {
        ...
        for (HashSet<Archer> archersByType : contestants.values()) {    //Iterating through all contestants (values only)
            for (Archer archer : archersByType) {                           //Iterating archersByType
                if (archer.getGender() == Archer.MALE && archer.getBow().getType().equals(Bow.BOW_TYPE_CARBON)) {
                    sortedByYearsExp.add(archer);
                }
            }
        }
    ...
    private void showSortedContestantsByName() {
        ...
        int position = 1;
        for (Archer archers : sortedArchersByName) {
            System.out.println((position++) + ":" + archers.getName());
        }
    }
